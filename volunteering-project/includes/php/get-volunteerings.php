...
<body>
    ...
    <div class="content">
        <div class="btn-group" style="width:100%">
          <button id="list-button" class="button-reset navigation-button" style="width:50%" onclick=changeToggleBarSelection('list')>List view</button>
          <button id="map-button" class="button-reset navigation-button" style="width:50%" onclick=changeToggleBarSelection('map')>Map view</button>
        </div>
        <!--<div id="debugging" style="position: absolute; background:red; width:500px; height: 500px; z-index:500"></div>-->
        <div id="map-element">
            <div id="map"></div>
        </div>
                                ...
                                if ($volunteerings_to_users_result = $conn-> query($get_volunteerings_to_users_table)) {
                                    if ($get_participants_of_volunteerings_result = $conn-> query($get_participants_of_volunteerings)) {
                                        while ($row = $get_volunteerings_details_result->fetch_assoc()) {
                                            ...
                                            $vol_datetime = new DateTime($date." ".$time);
                                            $current_datetime = new DateTime("now");

                                            if ($vol_datetime >= $current_datetime) {
                                            ...
                                            if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true)
                                            {
                                                ...
                                                if ($vol_datetime >= $current_datetime) {
                                                    ...
                                                    {
                                                        // User is already subscribed to this volunteeringn - Unsubscribe button
                                                        echo '<td><form action="remove-vol-subscription.php?source=list&user='.$_SESSION["user_id"].'" method="post"><input name="volunteeringid" type="text" style="display:none;" value="'.$id.'"><input id="'.$id.'" type="submit" class="btn btn-default" value="Unsubscribe" title="Unsubscribe"> </form></td>';
                                                    } else if ($is_volunteering_full) {
                                                        echo '<td><form action="get-volunteerings.php" method="post"><input name="volunteeringid" type="text" style="display:none;" value="'.$id.'"><input id="'.$id.'" type="submit" class="btn btn-default" value="Full" title="This volunteering is full" disabled> </form></td>';
                                                    } else {
                                                        $_SESSION["vol-title"] = $title;
                                                        $_SESSION["vol-description"] = $description;
                                                        $_SESSION["vol-date"]= $date;
                                                        $_SESSION["vol-start-time"] = $time;
                                                        $_SESSION["vol-duration"] = $duration;
                                                        echo '<td><form action="subscribe-volunteering.php" method="post"><input name="volunteeringid" type="text" style="display:none;" value="'.$id.'"><input id="'.$id.'" type="submit" class="btn btn-default" value="Subscribe"></form></td>';
                                                    }
                                                }
                                            }
                                        }
